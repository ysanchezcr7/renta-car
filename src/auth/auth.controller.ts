...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description: 'Crea un nuevo perfil de usuario (CUSTOMER u OWNER). Envía email de verificación.',
  })
  @ApiBody({ type: RegisterUserDto })
  @ApiResponse({ status: 201, description: 'Usuario creado. Se envió email de verificación.' })
  @ApiResponse({ status: 400, description: 'Datos inválidos o usuario ya existe' })
  @ApiResponse({ status: 403, description: 'Usuario existe pero no está verificado' })
  ...
  @ApiOperation({
    ...
    description: 'Autentica con email y contraseña. Genera OTP enviado por email.',
  })
  ...
  @ApiBearerAuth()
  @ApiOkResponse({ description: 'Perfil obtenido', type: ProfileUserResponseDto })
  @ApiResponse({ status: 401, description: 'No autenticado' })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado' })
  @Auth(Roles.OWNER, Roles.CUSTOMER)
  profile(@ActiveUser() user: UserActiveInterface): Promise<ProfileUserResponseDto> {
  ...
  @Auth(Roles.OWNER, Roles.CUSTOMER)
  async changePassword(@Body() dto: ChangePasswordDto, @ActiveUser() user: UserActiveInterface) {
    return this.authService.changePassword(user, dto);
  }

  @Post('request-password-reset')
  @ApiOperation({ summary: 'Solicitar restablecimiento de contraseña (envía OTP por email)' })
  ...
  @ApiQuery({ name: 'token', required: true, description: 'Token de verificación', type: String })
  ...
  async verifyEmail(@Query('token') encryptedToken: string, @Res() res: any) {
    try {
      ...
      const successUrl = process.env.VERIFY_EMAIL_SUCCESS_URL ?? 'http://localhost:3000/verify-email/success';
      return res.redirect(302, successUrl);
    } catch {
      const errorUrl = process.env.VERIFY_EMAIL_ERROR_URL ?? 'http://localhost:3000/verify-email/error';
